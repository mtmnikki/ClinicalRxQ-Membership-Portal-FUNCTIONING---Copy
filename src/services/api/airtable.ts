...
export class AirtableService {
  ...
  async getClinicalPrograms(): Promise<ClinicalProgram[]> {
    ...
    return records.map((r) => {
      const f = r.fields
      const programName = String(f[this.fields.programs.programName] || '')
      const slug = programNameToSlug(programName) as ProgramSlug
      const resourceCount = Array.isArray(f[this.fields.programs.linkedResourceId]) ? f[this.fields.programs.linkedResourceId].length : 0
